<form id="form-save" action="{{ route('dental_management.consultations.update', $consultation) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    @if(($isPatientLocked ?? false) && $consultation->patient_id)
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" value="{{ $patientInfo }}" readonly>
        </div>
    @else
        <div class="form-group">
            ...
            <select name="patient_id" id="patient_id" class="form-control" required>
                ...
                @foreach(\App\Models\Patient::all() as $patientOption)
                    <option value="{{ $patientOption->id }}" {{ (old('patient_id') ?? $consultation->patient_id) == $patientOption->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="treatment_id" id="treatment_id" class="form-control select2" required>
                    ...
                    @foreach($treatments ?? [] as $treatment)
                        <option value="{{ $treatment->id }}" {{ (old('treatment_id') ?? $consultation->treatment_id) == $treatment->id ? 'selected' : '' }}>
                            {{ $treatment->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="col-md-3">
            <div class="form-group">
                ...
                <input type="time" name="consultation_time" id="consultation_time" class="form-control"
                       value="{{ old('consultation_time', $consultation->consultation_time?->format('H:i')) }}" required>
            ...
            <div class="form-group">
                ...
                <div class="input-group">
                    <input type="number" name="fever" id="fever" class="form-control"
                           value="{{ old('fever', $consultation->fever) }}" step="0.1" min="30" max="45">
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <textarea name="description" id="description" class="form-control"
                          rows="2" maxlength="1000" required>{{ old('description', $consultation->description) }}</textarea>
                @error('description')
                    <div class="text-danger mt-1">{{ $message }}</div>
                @enderror
            </div>
