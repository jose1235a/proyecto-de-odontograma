...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (patientSearchButton) {
        patientSearchButton.addEventListener('click', function () {
            ...
            if (!documentValue) {
                updateFeedback('{{ __('dental_management.appointments.patient_search_help') }}', 'danger');
                return;
            }

            updateFeedback('{{ __('global.search') }}...', 'muted');
            fetch(searchUrl + '?document=' + encodeURIComponent(documentValue), {
                ...
                .then(data => {
                    ...
                    if (!patient) {
                        updateFeedback('{{ __('dental_management.appointments.messages.patient_not_found') }}', 'danger');
                        return;
                    }
                    patientIdInput.value = patient.id;
                    patientNameInput.value = patient.name;
                    patientNameDisplay.value = patient.name;
                    updateFeedback('{{ __('dental_management.appointments.messages.patient_found') }}', 'success');
                })
                .catch(() => {
